...
Original file is located at
    https://colab.research.google.com/drive/1xvhGs-uvbp4_XJAqgFIkNDiZP-eYWYSV
"""
...
def train_epoch(epoch, args, model, dataset_train, optimizer, scheduler, loss_fcn):
    ...
    for batch_data in dataset_train:
        ...
        for batch_idx, data in enumerate(batch_data):
           ...
           # print("neg Adj shape", neg_adj.shape)
            logits = model(features, pos_adj, neg_adj)
...
class Args:
    def __init__(self, gpu=0, subtask="regression"):
        ...
        # save model settings
        self.save_path = os.path.join(os.path.abspath('.'), "/mnt/data/sriram.kovela/THGNN/model_saved/")
...
def fun_train_predict(data_start, data_middle, data_end, pre_data):
    args = Args()
    print("args are", args.model_name)
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
    dataset = AllGraphDataSampler(base_dir="/mnt/data/sriram.kovela/THGNN/data_train_predict/", data_start=data_start,
                                  data_middle=data_middle, data_end=data_end)
    val_dataset = AllGraphDataSampler(base_dir="/mnt/data/sriram.kovela/THGNN/data_train_predict/", mode="val", data_start=data_start,
                                      data_middle=data_middle, data_end=data_end)
    ...
    print(val_dataset)
    data_code = os.listdir('/mnt/data/sriram.kovela/THGNN/daily_stock')
    #print("data code is ", data_code)
    data_code = sorted(data_code)
    data_code_last = data_code[data_middle:data_end]
    df_score=pd.DataFrame()
    for i in tqdm(range(len(val_dataset))):
        df = pd.read_csv('/mnt/data/sriram.kovela/THGNN/daily_stock/' + data_code_last[i], dtype=object)
    ...
    df_score.to_csv('/mnt/data/sriram.kovela/THGNN/prediction/pred.csv')
    print(df_score)

if __name__ == "__main__":
    data_start = 100
    data_middle =120
    data_end = data_middle+10
    pre_data = '2022-11-29'
